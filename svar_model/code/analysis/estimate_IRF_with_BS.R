...
rm(list = setdiff(ls(), c("By", "VAR.P", "CONST", 
                          "Y", "X", "hrz", 
                          "shock_sign", "num_var", 
                          "inv_tol", "confidence_level",
                          "Robustness")))
...
while(TRUE){
  ...
  B0_pseudo <- diag(diag(C_pseudo), ncol = num_var, nrow = num_var)
...
if (VAR.P == 1 & Robustness == 0) {
  saveRDS(df_IRF.sim, file = file.path(DROPBOX_PATH, 
                                       paste0("./data/intermediate_result/df_IRF.sim_hrz", 
                                              hrz + 1, "_", 
                                              confidence_level*100, "CI.rds")))
} else if (Robustness == 1) {
  saveRDS(df_IRF.sim, file = file.path(DROPBOX_PATH,
                                       paste0("./data/robustness/r1/df_IRF.sim_hrz", 
                                              hrz + 1, "_", 
                                              confidence_level*100, "CI.rds")))
} else if (Robustness == 2) {
  saveRDS(df_IRF.sim, file = file.path(DROPBOX_PATH,
                                       paste0("./data/robustness/r2/df_IRF.sim_hrz", 
                                              hrz + 1, "_", 
                                              confidence_level*100, "CI.rds")))
} else if (Robustness == 3) {
  saveRDS(df_IRF.sim, file = file.path(paste0("./data/robustness/r3/df_IRF.sim_hrz", 
                                              hrz + 1, "_", 
                                              confidence_level*100, "CI.rds")))
} else if (Robustness == 4) {
  saveRDS(df_IRF.sim, file = file.path(paste0("./data/robustness/r4/df_IRF.sim_hrz", 
                                              hrz + 1, "_", 
                                              confidence_level*100, "CI.rds")))
}
cat(">>> save simulation data\n")
